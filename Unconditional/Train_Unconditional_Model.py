...
# Define the neural network for score estimation
...
class LargeUNet(nn.Module):
    def __init__(self, time_embed_dim=128):
        ...
        # ---------------- Encoder ----------------
        self.enc1 = nn.Sequential(ResidualBlock(129, 64), ResidualBlock(64, 64))
...
def train(score_net, dataloader, optimizer, device, num_epochs=200, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in dataloader:
            x = x.to(device)
            epsilon = 1e-3
            t = torch.rand(x.size(0), device=device)  *(1-epsilon) +epsilon
                        
            # Compute loss
            optimizer.zero_grad()
            loss = dsm_loss(score_net, x, t, device) 
            loss.backward()
            optimizer.step()
